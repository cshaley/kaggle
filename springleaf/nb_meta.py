...
def fit_train(datapath,rf,max_no_rows,constants,strings,encoders):
	...
	ndf['sem']=X.sem(axis=1)
        ...
        other_runs=["kn2.csv","kn4.csv","kn8.csv","kn16.csv","kn32.csv",\
                    "kn64.csv","kn128.csv","kn256.csv","kn512.csv","kn1024.csv","et100-40.csv","rf10-100-40.csv"]
        p=pd.read_csv("old/xgbout-0.01-10-2000-0-45-10-0.2325-auc-0.1-1-0.4.csv").set_index("ID")
        
        p['target0']=p['target']
        p=p.drop('target',1)
        
        for count,fl in enumerate(other_runs):
            n=pd.read_csv(fl).set_index("ID")
            p=p.join(n,lsuffix=str(count))
...
#            df[col+'quarter']=pd.to_datetime(df[col],format='%d%b%y:%H:%M:%S').dt.quarter
        ...
        ndf['prod']=X1.prod(axis=1)
        #df=df.join(ndf).join(p)
        df=ndf.join(p)
        
        if sample_no == 0:
            submission = pd.DataFrame(rf.predict_proba(df)[:,1], index=df.index, columns=['target'])
...
logfile = basepath+'code/xgmeta_encode.log'
                                ...
                                for al in alphas:
                                    for la in lambdas:
                                        for cs in col_subsamples:
                                            ...
                                            xgcl = xg.XGBoostClassifier().set_params(num_boost_round=ne,num_class=2,nthread=4,max_depth=md,learning_rate=lr,gamma=gm,min_child_weight=mcw,max_delta_step=mds,base_score=bs,eval_metric=em,colsample_bytree=1,alpha=al)
            #eval_metric='auc'
                                            ...
                                            print "It took " +str((time.time()-start_time)/60.0)+" minutes to execute."
